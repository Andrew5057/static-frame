...
skip_np_no_float128 = pytest.mark.skipif(
    not hasattr(np, 'float128'), reason='NumPy does not have float128'
)
...
def temp_file(
    suffix: tp.Optional[str] = None, path: bool = False
...
class TestCase(unittest.TestCase):
    ...
    def get_test_db_b() -> sqlite3.Connection:
        ...
        for identifier in ('a1', 'b2'):
            for date in ('2006-01-01', '2006-01-02'):
                c.execute(
                    f"INSERT INTO events VALUES ({count}, '{date}','{identifier}',12.5,8)"
                )
                count += 1
        conn.commit()
        return conn
    ...
    def get_test_db_e() -> sqlite3.Connection:
        ...
        for date in ('2006-01-01', '2006-01-02'):
            for identifier in ('a1', 'b2'):
                c.execute(
                    f"INSERT INTO events VALUES ('{date}','{identifier}',12.5,{count})"
                )
                count += 1
        conn.commit()
        return conn
    ...
    def get_test_db_f() -> sqlite3.Connection:
        ...
        for date in ('2006-01-01', '2006-01-02'):
            for identifier in ('a1', 'b2'):
                c.execute(
                    f"INSERT INTO events VALUES ({count},'{date}','{identifier}',12.5)"
                )
                count += 1
        conn.commit()
        return conn
    ...
    def assertEqualWithNaN(
        ...
        if (
            (isinstance(v1, float) or isinstance(v1, np.floating))
            and np.isnan(v1)
        ...
        if (
            (isinstance(v1, complex) or isinstance(v1, np.complexfloating))
            and cmath.isnan(v1)
        ...
        if (
            isinstance(v1, np.datetime64)
            and np.isnan(v1)
            and isinstance(v2, np.datetime64)
            and np.isnan(v2)
        ):
            return

        if (
            isinstance(v1, np.timedelta64)
            and np.isnan(v1)
            and isinstance(v2, np.timedelta64)
            and np.isnan(v2)
        ):
            return
    ...
    def assertTypeBlocksArrayEqual(
        self,
        tb: TypeBlocks,
        match: tp.Iterable[tp.Any],
        match_dtype: tp.Optional[tp.Union[type, TDtypeAny, str]] = None,
    ) -> None:
        """
        Args:
    ...
    def assertAlmostEqualValues(
        self, values1: tp.Iterable[tp.Any], values2: tp.Iterable[tp.Any]
    ) -> None:
        for v1, v2 in zip_longest(values1, values2):
            self.assertEqualWithNaN(v1, v2)

    def assertAlmostEqualItems(
        self,
        pairs1: tp.Iterable[tp.Tuple[TLabel, tp.Any]],
        pairs2: tp.Iterable[tp.Tuple[TLabel, tp.Any]],
    ) -> None:
    ...
    def assertAlmostEqualFramePairs(
        self,
        pairs1: tp.Iterable[tp.Tuple[TLabel, tp.Iterable[tp.Any]]],
        pairs2: tp.Iterable[tp.Tuple[TLabel, tp.Iterable[tp.Any]]],
    ) -> None:
        """
    ...
    def assertEqualFrames(
        self, f1: TFrameAny, f2: TFrameAny, compare_dtype: bool = True
    ) -> None:
        if not f1.equals(f2, compare_dtype=compare_dtype):
            self.assertTrue(
                f1.index.equals(f2.index, compare_dtype=compare_dtype),
                'index do not match',
            )
            self.assertTrue(
                f1.columns.equals(f2.columns, compare_dtype=compare_dtype),
                'columns do not match',
            )
            self.assertTrue(
                f1._blocks.equals(f2._blocks, compare_dtype=compare_dtype),
                '_blocks do not match',
            )
